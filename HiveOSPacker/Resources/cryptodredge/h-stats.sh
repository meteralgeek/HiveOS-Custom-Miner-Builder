...
. /hive/custom/cryptodredge/h-manifest.conf
	...
	if [[ $? -ne 0  || -z $threads ]]; then
		echo -e "${YELLOW}Failed to read $miner stats from localhost:${WEB_PORT}${NOCOLOR}"
	else
		summary=`echo "summary" | nc -w 2 localhost ${WEB_PORT}`
		...
		[[ ! -z $CUSTOM_ALGO ]] && algo=$CUSTOM_ALGO || algo=`echo "$summary" | tr ';' '\n' | grep -m1 'ALGO=' | sed -e 's/.*=//'`
